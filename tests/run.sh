...
# This runner ships inside the pascal repo at tests/. The .pas corpus
# (tests/Pascal), the compiler regression suite (tests/compiler/pascal), the
# shared harness + CLI fixtures (tests/tools) and the runtime units (tests/lib)
# are all vendored beside it, so the repo runs self-contained with no umbrella
# tree. PASCAL_BIN defaults to the standalone build (../build/pascal); callers
# can override it, e.g. the umbrella points it at build/bin/pascal over this same
# corpus.
TESTS_DIR="$SCRIPT_DIR"
EX_DIR="$(cd "$SCRIPT_DIR/../examples" && pwd)"
PASCAL_BIN="${PASCAL_BIN:-$SCRIPT_DIR/../build/pascal}"
export PASCAL_BIN
...
run_pascal_fixture() {
    ...
    # The HTTP/API fixtures call the libcurl-backed networking builtins
    # (HttpSession/HttpRequest, apiSend/apiReceive). A minimal pscal-core build
    # omits libcurl, so those builtins fail at runtime. Skip them when the
    # capability is absent, the same way the SocketSendReceiveTest .net marker is
    # gated. Forcing RUN_NET_TESTS=1 opts them back in when curl is present.
    case "$test_name" in
        ApiSendReceiveTest|HttpAsyncFileURL|HttpFetchDataURL|HttpHeadersFileURL|HttpRequestToFileFileURL)
            if [ "$PASCAL_HTTP_AVAILABLE" -ne 1 ] && [ "${RUN_NET_TESTS:-0}" != "1" ]; then
                harness_report SKIP "pascal_${test_name}" "Pascal fixture $test_name" "libcurl-based networking unavailable"
                return
            fi
            ;;
    esac
...
# Runtime units are vendored at tests/lib (mirrors the umbrella's lib/). The
# harness copies them into a writable scratch root because the binary expects a
# crt.pl unit (sourced from crtvt.pl) and because an installed tests/lib could be
# read-only. Default the scratch root to a temp dir; honour PASCAL_LIB_DIR.
VENDOR_LIB="$TESTS_DIR/lib"
CRT_UNIT="$VENDOR_LIB/pascal/crtvt.pl"
if [ -n "${PASCAL_LIB_DIR:-}" ]; then
    PASCAL_LIB_ROOT="$PASCAL_LIB_DIR"
else
    PASCAL_LIB_ROOT="$(mktemp -d)"
    trap 'rm -rf "$TEST_HOME" "$PASCAL_LIB_ROOT"' EXIT
fi
PASCAL_LIB_DIR="$PASCAL_LIB_ROOT/pascal"
export PASCAL_LIB_DIR
export PSCAL_LIB_DIR="$PASCAL_LIB_DIR"
mkdir -p "$PASCAL_LIB_ROOT"
cp -R "$VENDOR_LIB/." "$PASCAL_LIB_ROOT/"
if [ -f "$CRT_UNIT" ]; then
    cp "$CRT_UNIT" "$PASCAL_LIB_DIR/crt.pl"
fi

if grep -q '^SDL:BOOL=ON$' "$SCRIPT_DIR/../build/CMakeCache.txt" 2>/dev/null; then
    SDL_ENABLED=1
else
    SDL_ENABLED=0
fi
...
# The HTTP/API builtins are core VM builtins (not ext-builtins), so they do not
# show up in --dump-ext-builtins. Probe them directly: a minimal pscal-core build
# compiled without libcurl reports "this build omits libcurl-based networking"
# when HttpSession is called, and exits non-zero. A build with curl runs it.
probe_http_available() {
    local probe_src probe_err
    probe_src=$(mktemp)
    probe_err=$(mktemp)
    cat > "$probe_src" <<'PROBE'
program HttpProbe;
...
PROBE
    set +e
    "$PASCAL_BIN" --no-cache "$probe_src" >/dev/null 2>"$probe_err"
    local rc=$?
    set -e
    rm -f "$probe_src"
    if [ $rc -eq 0 ] && ! grep -q 'omits libcurl-based networking' "$probe_err"; then
        rm -f "$probe_err"
        return 0
    fi
    rm -f "$probe_err"
...
if probe_http_available; then
    PASCAL_HTTP_AVAILABLE=1
else
    PASCAL_HTTP_AVAILABLE=0
fi
