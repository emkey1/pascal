...
# This suite is vendored verbatim into the standalone pascal repo under
# tests/compiler/pascal/. The umbrella points PASCAL_BIN at build/bin/pascal over
# the same manifest; the standalone build sets PASCAL_BIN to its own binary via
# CTest. Honour that env override and fall back to the umbrella default.
PASCAL_BIN = Path(os.environ.get("PASCAL_BIN", str(REPO_ROOT / "build/bin/pascal")))
MANIFEST_PATH = Path(__file__).resolve().with_name("manifest.json")
# Directory holding this runner+manifest (.../compiler/pascal). Manifest paths are
# written repo-relative ("Tests/compiler/pascal/cases/x.pas"); in the standalone
# layout that prefix is stripped and the case is resolved beside the manifest.
SUITE_DIR = Path(__file__).resolve().parent
MANIFEST_PREFIX = "Tests/compiler/pascal/"


def resolve_source(path_value):
    """Locate a manifest case across both the umbrella and standalone layouts."""
    candidate = (REPO_ROOT / path_value).resolve()
    if candidate.exists():
        return candidate
    rel = path_value
    if rel.startswith(MANIFEST_PREFIX):
        rel = rel[len(MANIFEST_PREFIX):]
...
def run_test(entry):
    ...
    source_path = resolve_source(path_value)
